...
To ensure that the experiments were consistent and reproducible, we used a fixed experimental protocol throughout the project.

### Dataset and Data Splits

* **Dataset:** PASCAL VOC 2012
* **Number of object classes:** 20
* **Training split:** PASCAL VOC 2012 training set
* **Evaluation split:** PASCAL VOC 2012 validation set
* **Image format:** RGB
* **Ground-truth annotations:** Object class labels, bounding boxes, and segmentation masks

For the deep-learning experiments, the ResNet50 model was trained using **4,500 training images** and validated using **800 validation images**.

The final controlled fine-tuning comparison was evaluated on the same fixed set of **400 validation images** under clean and distorted conditions.

For the classical computer vision tasks, a fixed subset of validation images containing the required annotations was used. The same image indices were maintained when comparing clean, distorted, and restored images.

### Image Preprocessing

All images used as input to ResNet50 were:

* Converted to RGB
* Resized to the model input resolution
* Converted to PyTorch tensors
* Normalized using the ImageNet mean and standard deviation

The classical Canny and GrabCut algorithms were applied to the original-resolution images.

### Canny Edge Detection

Canny Edge Detection was applied to grayscale images.

Because PASCAL VOC does not provide ground-truth edge annotations, the clean-image Canny edge map was used as the reference for the distortion and restoration experiments.

Performance was measured using Edge-Map IoU between the clean reference edge map and the edge map produced from the distorted or restored image.

The edge density reported for clean images is a descriptive measurement of the proportion of edge pixels and is not treated as a ground-truth accuracy metric.

### GrabCut Foreground Segmentation

GrabCut was initialized using the ground-truth bounding box of the largest annotated object in each image.

The resulting foreground mask was compared with the corresponding PASCAL VOC ground-truth segmentation mask.

Performance was measured using Intersection over Union:
...
The ResNet50 model was initialized with ImageNet pretrained weights and adapted to output predictions for the 20 PASCAL VOC classes.

Because multiple object classes may appear in the same image, the task was treated as multi-label classification.

The model outputs were converted to probabilities using the sigmoid function, and a prediction threshold of **0.30** was used.

The main classification metric used in the distortion, restoration, and fine-tuning experiments was the sample-averaged multi-label F1-score.

Multi-label Jaccard score was also reported in the initial clean-image baseline analysis as an additional metric.

### Distortion Protocol

Three distortion types were evaluated:

1. Salt & Pepper Noise
2. Overexposure
3. Motion Blur

Each distortion was applied at six increasing severity levels, from **L1** to **L6**.

For every distorted image, the Signal-to-Noise Ratio was calculated relative to the corresponding clean image.

Lower SNR values indicate stronger image degradation.

### Controlled Comparisons

Whenever two approaches were compared, the following conditions were kept fixed:

* The same original image indices
* The same ground-truth labels and masks
* The same evaluation threshold
* The same evaluation metrics
* The same distortion parameters
* The same random seed where applicable

This ensured that changes in performance were caused by the evaluated distortion, restoration method, or fine-tuning strategy rather than by differences in the evaluation data.
